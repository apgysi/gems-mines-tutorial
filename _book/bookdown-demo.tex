...
\emph{Do not forget, you have an extensive list of minerals included in this database. Once you have gone through the tutorials and are familiar with GEMS, I suggest that in thermodynamic database mode you switch to the \texttt{Phase} module (\autoref{fig:window6.jpeg}), and delete minerals that are not relevant for your own specific project. Also, for less advanced users, I suggest not using the feldspar solid solution model (ss) but only their end members (i.e., anorthite, albite and microcline).}
...
\includegraphics[width=0.7\linewidth]{figures/module1/fig-5} \textbackslash caption\{Select 3rdparty/mines and support, and for phases select only withoutss. \emph{Note that for advanced users you can choose withss; the expanded tab shows pure for endmembers and ss for different solid solution endmembers}\}\label{fig:fig-5}
