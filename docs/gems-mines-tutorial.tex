...
\includegraphics[width=0.5\linewidth]{figures/top-pic}
...
\begin{figure}
\includegraphics[width=8.5\linewidth]{figures/module3/fig-12} \caption{Selection of phase or phase endmembers for solid solutions. For a Topaz solid solution we select the OH and F endmembers. Tick Full-list selection to see the list of available records.}\label{fig:fig-12c}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{figures/module3/fig-17} \caption{Choose the results to be plotted. Here we choose the mass minerals (in gram) using phM. Note that you can already select cNu for the x-axis, by switching to scalar and righ-clicking on cNu, then select abscissa.}\label{fig:fig-17c}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/module5/fig-2} \caption{GEM2MT module Initial window page showing the steps to set up 1) the initial name of rock and fluid, 2) pressure and temperature, 3) input quantity of rock and fluid in kg, 4) compositions of rock and fluid (i.e., row index 0 corresponds to the fluid and row index 1 to rock), and 5) input of additional user-defined formulae and units (these fields can be added/selected in the wizard window).}\label{fig:fig-2e}
\end{figure}
...
\hypertarget{create-a-parent-record}{%
\section{Create a parent record}\label{create-a-parent-record}}

The first step is to setup a parent system record in \texttt{SysEq}.

\begin{itemize}
\item
  Select the \texttt{SysEq} option in the left panel and create and select `Create a new record from scratch'.
\item
  We will create a new record NdPO4, using a temperature of 100 \(^\circ\)C and a pressure of 0 (psat).
\item
  For the input recipe we need to add our acid (HCl) and base (NaOH), which will be adjusted later in the \texttt{Process} simulations for the inverse pH titration. Figure \ref{fig:fig-1f} shows the composition of the system. Make sure to use the unit `M' for moles acid/base.
\end{itemize}

\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/module6/fig-1} \caption{Input recipe in 'SysEq' which we need to add our acid (HCl) and base (NaOH) to setup our parent system record to setup an inverse titration problem}\label{fig:fig-1f}
\end{figure}

\hypertarget{setting-up-an-acidbase-titration}{%
\section{Setting up an acid/base titration}\label{setting-up-an-acidbase-titration}}

\begin{itemize}
\item
  Now we switch on the left panel and select the \texttt{Process} simulation mode and `Create a new record from scratch'. We can call it pH-activity and select the \texttt{Process\ simulation\ code} (G) for the type of process to simulate as shown in Figures \ref{fig:fig-2f}.
\item
  In the next window, first choose a model (\texttt{Property-vs-pH\ diagram}), and in sequence select first the acid (HCl) then the base (NaOH), which should replace the code in the right window. Make sure to select a temperature (e.g.~100 \(^\circ\)C) and pressure (0 for psat).
\item
...
\item
  Select items to be plotted (\texttt{Scalars}: pH; \texttt{lga}: aqueous Nd species; `lgm\_t' dissolved Nd, as shown in Figure \ref{fig:fig-4f}.
\item
  Accept all the following dialogues. Then click on \texttt{Save\ this\ record\ to\ database}, which creates your new process simulation record.
\item
...
\tightlist
\item
  Then click on the calculator icon \texttt{Re-calculate\ and\ check\ record\ data} to display the graph. Figure \ref{fig:fig-5f} shows the resulting pH-activity diagram for Nd speciation.
\end{itemize}

\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/module6/fig-2} \caption{In `Process` we create a new record from scratch and select the `Process simulation code` (G) for inverse titration.}\label{fig:fig-2f}
\end{figure}
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/module6/fig-3} \caption{In this wizard step we select the properties of the model.}\label{fig:fig-3f}
\end{figure}

\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/module6/fig-4} \caption{In this wizard step we select the variables to be plotted. pH is found under 'Scalars', log activity of Nd species und 'lga' and total dissolved log molality Nd under 'lgm'. Note to select pH for the x-axis, right click on it first, then select 'Abscissa', then select the variables to plot on the y-axis.}\label{fig:fig-4f}
\end{figure}

\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/module6/fig-5} \caption{pH-acitivty plot showing the total dissolved molality of Nd as a function of pH and the log activity of acqueous Nd species. In this example Nd3+ and Nd chloride species are predominant at acidic pH and Nd hydroxyl species at higher pH. Note that the Nd hydroxyl species originate from the Supcrt92 database, and need to be updated in the future. The species are represented as anhydrous compounds but can be related to the more common stoichiometric formula, for e.g. NdO2H0 + H2O = Nd(OH)30.}\label{fig:fig-5f}
\end{figure}
...
\includegraphics[width=0.25\linewidth]{figures/NSFlogo}

\includegraphics[width=0.25\linewidth]{figures/NMTbureau}

\includegraphics[width=0.25\linewidth]{figures/NMTlogo}
