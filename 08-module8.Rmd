...
```{r,fig-2, echo = FALSE, out.width="80%", fig.cap="`SysEq` window showing how to create a new parent record. Select basalt and seawater from the MINES thermodynamic database, listed under `Compos`, and select base and precious metals under `IComp` and add the respective concentrations."}
knitr::include_graphics("figures/module8/fig-2.png")
```
...
```{r,fig-3, echo = FALSE, out.width="80%", fig.cap="`SysEq` window showing the modeling results for basalt alteration minerals (top) and the input recipe window (yellow vial) to add kinetic metastability constrain `Kin.upper(dul_)` for saponite (bottom)."}
knitr::include_graphics("figures/module8/fig-3.png")
```
...
```{r,fig-5, echo = FALSE, out.width="80%", fig.cap="Top shows the parent record created in `SysEq` with seawater and basalt added. The bottom window shows the selection of this parent record when creating a new record in the `Process simulation` mode for the titration model (`S mode`)."}
knitr::include_graphics("figures/module8/fig-5.png")
```
...
```{r,fig-6, echo = FALSE, out.width="80%", fig.cap="(Step 2) Titration model setup in `S mode` in the `Process simulation` mode. Make sure to (1) select the correct model, (2) select the titrant, here we use basalt, (3) select the initial, final and steps of basalt to be added, (4) add the correct pressure and temperature (here we use a constant P-T, but steps and gradients can be scripted). (Step 3) Variables to be selected for plotting, here we choose first `Scalars`, right click and choose abscissa for cNu which is grams rock added, then switch to `phVol` to select phase volumes in cm3. (step 4) Tick `Volume V, I ('vV')` for allocating an optional data vectors "}
knitr::include_graphics("figures/module8/fig-6.png")
```
...
```
```{r,fig-7, echo = FALSE, out.width="75%", fig.cap="`Process simulation mode` with `Controls` window used for the titration script setup and the `Sampling` script to plot up variables. Here we use a custom script which can be copy-pasted from the text into your script windows."}
knitr::include_graphics("figures/module8/fig-7.png")
```
...
```{r,fig-8, echo = FALSE, out.width="80%", fig.cap="Seawater-basalt interaction simulation results showing the volume % of minerals as a function of logarithm of the fluid/rock ratios (F/R)."}
knitr::include_graphics("figures/module8/fig-8.png")
```
...
```{r,fig-9, echo = FALSE, out.width="80%", fig.cap=" (Step 2) Setup of singlepass flushing and heating model (`R mode`): (1) select flushing, (2) select the basalt, (3) seawater, (4) a temperature increase from 25 to 400 &deg;C at 500 bar. (Step 3) Select the log molality elements to be plotted under `logm_t`; use temperature for the x-axis under `Scalar`."}
knitr::include_graphics("figures/module8/fig-9.png")
```
...
```{r,fig-10, echo = FALSE, out.width="75%", fig.cap="Singlepass flushing and heating model showing the logarithm molality of total dissolved elements in the reacted seawater, pH, eH and activities of OH^-^ and H^+^. This model was setup with 5 g of basalt in each box, 1000 g of reactive seawater and a temperature increase from 25 to 400 &deg;C at 500 bar."}
knitr::include_graphics("figures/module8/fig-10.png")
```
## Outcomes
